...
for batch_size in [50, 100, 200, 400, 800]:
    ...
    optimizer = optim.SGD(net.parameters(), lr=0.1, momentum=0.5, weight_decay=5e-4)
    ...
    plt.plot(range(n_epochs), loss_train, label=f"Train_batch={batch_size}")
    plt.plot(range(n_epochs), loss_test, label=f"Validation_batch={batch_size}")
